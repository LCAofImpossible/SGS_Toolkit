...
Cerca le attività per una o più parole chiave. Per termini generici come `plastica`, `granulo di plastica` e `polietilene`, la ricerca restituisce famiglie di materiale alternative presenti nel catalogo, distinguendo anche l'origine riciclata quando indicata. Questi risultati non hanno un punteggio di adeguatezza: occorre specificare il polimero e il grado per scegliere. Per ricerche specifiche, il ranking privilegia i termini nel nome dell'attività, poi reference product se disponibile; settore e descrizione contribuiscono solo in misura minore. Puoi filtrare per geografia, unità e tipo di attività, oppure indicare una fase preferita. I risultati raggruppano le varianti geografiche della stessa attività e mostrano il motivo del punteggio. Nel BOM Mapper seleziona un dataset per fase. Puoi marcare trasformazione e finitura come **già incluse** o **non applicabili**; scrivi sempre la motivazione quando la scelta non è evidente. Mapping QA segnala passaggi mancanti, unità non in kg, disallineamenti geografici e possibili sovrapposizioni. Esporta un CSV di mappatura con metadati e rilievi oppure salva/riapri un progetto JSON.

### Combinazioni candidate per un processo reale

Nella sezione **Dataset Finder → Combinazioni candidate**, scrivi soltanto la descrizione del prodotto o processo reale, ad esempio `cover in ABS stampato a iniezione`, `lamiera in acciaio laminata e zincata` oppure `acciaio inox in altoforno`. Premi **Trova combinazioni**: il tool interpreta automaticamente materiale, trasformazione, finitura e forma, poi mostra i dataset candidati e le ipotesi che richiedono verifica. Se un termine non è riconosciuto, puoi riformulare la descrizione.

Il generatore restituisce fino a tre catene distinte per un materiale identificato, se disponibili: prodotto semilavorato già modellato (attività o mercato), materiale più servizio di trasformazione, oppure un mercato del servizio. Se il materiale è indicato solo come `plastica` o `polietilene`, mostra invece più ipotesi di polimero o grado e, se riconosce una lavorazione, la abbina separatamente a ciascuna. Per `granulo di plastica` comprende PP, PE-HD, PE-LD, PET e una variante riciclata non specificata quando presenti nel catalogo. Sono esempi e non un elenco esaustivo né una scelta automatica del materiale reale. Usa il campo `Product Information` del catalogo per distinguere servizi che escludono il materiale dai prodotti che potrebbero già includerlo. Quando il confine del servizio non è esplicito, segnala la verifica necessaria. Una finitura con input di materiale incluso o riferita a una forma incompatibile non viene aggiunta automaticamente. Vengono segnalate le unità diverse, per esempio `kg` e `m²`. Le combinazioni si possono esportare in CSV. Nel caso `acciaio inox in altoforno`, il tool non somma la ghisa da altoforno all'acciaio inox: propone il mercato dell'inox e la produzione elettrica come alternative presenti nel catalogo, evidenziando l'incertezza sulla rotta richiesta.
